...
-- |0><0| = 1/2 (I + Z)
proj0 :: VarId -> Summed ScaledPauli
proj0 v = scaleSummed 0.5 (add (pauliI v) (pauliZ v))

-- |1><1| = 1/2 (I - Z)
proj1 :: VarId -> Summed ScaledPauli
proj1 v = scaleSummed 0.5 (sub (pauliI v) (pauliZ v))

-- |1><0| = 1/2 (X - iY)
raise :: VarId -> Summed ScaledPauli
raise v = scaleSummed 0.5 (sub (pauliX v) (scale (0 :+ 1) (pauliY v)))

-- |0><1| = 1/2 (X + iY)
lower :: VarId -> Summed ScaledPauli
lower v = scaleSummed 0.5 (add (pauliX v) (scale (0 :+ 1) (pauliY v)))

-- Single-qubit factor of |m_bit><j_bit|
bitTransition :: Int -> Int -> VarId -> Summed ScaledPauli
bitTransition 0 0 = proj0
bitTransition 1 1 = proj1
bitTransition 0 1 = raise
bitTransition 1 0 = lower
bitTransition j m = error $ "bitTransition: invalid bits " ++ show (j, m)
...
toPauli totalChoiceCount i = \x ->
  let
      ...
      pauliOps = zipWith (\bit qubitId -> if bit == 1 then proj1 qubitId else proj0 qubitId) bits qubitIds
  in Tensor pauliOps

-- |m><j| as a tensor of single-qubit factors on variable `base`
ketBra :: Int -> Int -> Int -> VarId -> Tensor (Summed ScaledPauli)
ketBra n j m base =
  let jBits = toBits n j
      mBits = toBits n m
      qubitIds = [base * n + u | u <- [0 .. n - 1]]
  in Tensor (zipWith3 bitTransition jBits mBits qubitIds)

expandKetBra :: Tensor (Summed ScaledPauli) -> Summed (Scaled (Tensor PauliExpr))
expandKetBra = fmap floatScalars . distr
...
addK cSize k base
  ...
  | otherwise =
      let n = neededBitSize cSize
          terms =
            [ expandKetBra (ketBra n j (j + k) base)
            | j <- [0 .. cSize - k - 1]
            ]
...
relationalZeroSites cSize sites =
  combine $ joinSummed $ Summed
    ( termZero : [ termK k | k <- [1 .. cSize - 1] ] )
  where
    -- Calculate the identity shift exactly once (k=0) 
    termZero = composeSites (map (addK cSize 0) sites)

    -- Calculate both +k and -k directions for all actual shifts (k > 0)
    termK k =
      combine $ joinSummed $ Summed
...
-- | All sites in u shift by distinct (k, j) pairs
relationalOneSites
  :: Int -> [VarId] -> Summed (Scaled (Tensor PauliExpr))
...
-- | All sites in u shift by (k, j) where j is a permutation of k
relationalSwapSites
  :: Int -> [VarId] -> Summed (Scaled (Tensor PauliExpr))
relationalSwapSites _cSize [] = Summed []
relationalSwapSites cSize sites =
  combine $ joinSummed $ Summed
    [ composeSites
        [ compareShift cSize ki ji site
        | (site, ki, ji) <- zip3 sites kTuple jTuple
...
relationalSwap
  :: Int -> Int -> [VarId] -> Summed (Scaled (Tensor PauliExpr))
relationalSwap cSize arity allSites
  | arity <= 0 = Summed []
  | otherwise =
      combine $ joinSummed $ Summed
        [ relationalSwapSites cSize u
        | u <- combinations arity allSites
        ]
